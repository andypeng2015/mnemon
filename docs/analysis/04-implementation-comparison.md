...
| **LLM 依赖** | gpt-4o-mini (必需) | Claude Haiku/Sonnet 子代理 (可选) | 无直接依赖 |
...
| **外部依赖** | OpenAI API Key 必需 | 核心仅 mcp+pydantic | 零 (纯 Go 二进制) |
...
| **LLM 增强** | 核心路径 (必需) | 可选 (entity-extractor 子代理) | 外部 (Claude --entities flag) |
| **成本** | 每条 ~$0.001 (gpt-4o-mini) | 可选 ~$0.0003 (Haiku) | $0 |
...
### Mnemon：CLI-in-the-loop（LLM 在管道外部）

```
Claude ──CLI调用──→ mnemon remember/recall/diff/link/gc
   ↑                    ↓
   └── JSON 输出 ───────┘
       (candidates, signals, hints)
```

- LLM **完全在管道外部**，mnemon 二进制零 LLM 依赖
- mnemon 输出 candidates 和 signals，Claude 在自己的 context window 中评估
- Claude 的评估质量可能高于 gpt-4o-mini（更大模型 + 更完整上下文）
- 独立于任何特定 LLM 提供商
...
|------|----------------|---------------------|---------------------|
| **图质量** | 最高（LLM 提取+推断） | 中高（可选 LLM + regex） | 中（regex + Claude 补充） |
| **写入延迟** | 高（每次 LLM 调用 ~1-3s） | 中低（regex 为主） | 最低（纯计算） |
| **写入成本** | ~$0.001-0.003/条 | ~$0-0.001/条 | $0 |
| **查询质量** | 最高（LLM 参与检索+生成） | 中（搜索为主） | 中高（signals 辅助 Claude 判断） |
| **独立性** | 绑定 OpenAI | 绑定 Claude (MCP) | 完全独立 |
...
1. **CLI-in-the-loop**：比 MCP 更灵活（不绑定特定 LLM 框架），比 LLM-in-pipe 更经济
2. **零 API 依赖**：是最大的差异化优势
3. **单二进制**：Go 编译的部署优势不可替代
4. **SQLite 统一存储**：简洁性 > 性能（在 1000 insight 规模下）
5. **Signals 元数据**：这是 MAMGA 和 memcp 都没有的，让 Claude 能做更好的判断
